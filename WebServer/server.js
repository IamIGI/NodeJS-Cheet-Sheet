...
const serveFile = async (filePath, contentType, response) => {
    try {
        ...
        response.end(
            contentType === 'application/json' ? JSON.stringify(data) : data //json files
        );
    } catch (err) {
        console.log(err);
...
const server = http.createServer((req, res) => {
    ...
    //makes .html extension don't required in the browsers address
    ...
    if (fileExists) {
        ...
        switch (path.parse(filePath).base) {
            case 'old-page.html':
                res.writeHead(301, { Location: 'new-page.html' }); //redirect have status code 301
                res.end();
                break;
            case 'www-page.html':
